...
\usepackage{tabularx}
\usepackage{longtable}
\author{STEVEN ECHEVERRÍA}
\date{2026-04-08}
\title{Taller de Configuración de Entorno WSL}
\hypersetup{
 pdfauthor={STEVEN ECHEVERRÍA},
...
\endlastfoot
\hline
\texttt{C-e} & Mover el cursor al final de una línea & \texttt{C-y} & Pegar un texto copiado o borrado anteriormente\\[0pt]
\texttt{C-x-1} & Quedarme solo con la ventana activa y eliminar las demás & \texttt{C-x b} & Abrir otro buffer\\[0pt]
\texttt{M-k} & Eliminar una oración completa & \texttt{M-C-v} & Desplazar el buffer que se encuentra en otra ventana\\[0pt]
\end{longtable}
\subsection{Comandos Emacs Juego}
\label{sec:orgffc133e}
...
\begin{center}
\includegraphics[width=.9\linewidth]{./images/StevenEcheverria1.png}
\end{center}
\begin{center}
\includegraphics[width=.9\linewidth]{./images/StevenEcheverria2.png}
\end{center}
\begin{center}
\includegraphics[width=.9\linewidth]{./images/StevenEcheverria3.png}
\end{center}
\subsection{Usando Emacs para tener Ayuda sobre Emacs}
\label{sec:org4b02519}
...
\item \textbf{\textbf{Estudiante D:}} \texttt{C-p}. C-p runs the command previous-line (found in global-map), which is an
\end{enumerate}
...
\item \textbf{\textbf{Estudiante B:}} <Poner el comando>. <Pegar lo que dice el primer párrafo de la ayuda>
\item \textbf{\textbf{Estudiante C:}} <Poner el comando>. <Pegar lo que dice el primer párrafo de la ayuda>
\item \textbf{\textbf{Estudiante D:}} \textasciitilde{}C-x 4 C-f. C-x 4 C-f runs the command find-file-other-window (found in
\end{enumerate}
...
\hline
Alan Turing & a.turing@enigma.com & líder\\[0pt]
Steven Echeverría & steven.echeverria@epn.edu.ec & colaborador\\[0pt]
 &  & \\[0pt]
\hline
\end{tabular}
\end{center}

\section{Verificación de Entregables [85\%]:}
\label{sec:org2e90098}
